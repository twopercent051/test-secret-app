...
@router.post(path="/")
async def create_secret_endpoint(secret: SetSecretSchema, ip: str = Depends(get_client_ip)
) -> GetSecretKeySchema:
    encrypted_passphrase = encrypt_passphrase(passphrase=secret.passphrase)
    encrypted_secret = encrypt_secret(secret=secret.secret)
    created_secret = await SecretsDAO.create_secret_with_log(encoded_secret=encrypted_secret,
                                                             encoded_passphrase=encrypted_passphrase, ttl_seconds=secret.ttl_seconds, ip=ip)
...
async def delete_secret_endpoint(secret_key: int, passphrase: str, ip: str = Depends(get_client_ip)) -> DeleteSecretSchema:
    encrypted_passphrase = encrypt_passphrase(passphrase=passphrase)
    is_deleted = await SecretsDAO.delete_secret_with_log(secret_id=secret_key, encoded_passphrase=encrypted_passphrase, ip=ip)
    if not is_deleted:
        raise HTTPException(status_code=404, detail="Secret not found")
